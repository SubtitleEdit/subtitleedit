        ...
        public static string AutoBreakLine(string text, int maximumLength, int mergeLinesShorterThan, string language)
        {
            ...
            if (text.Contains(Environment.NewLine) && (text.Contains('-') || text.Contains('♪')))
            {
                ...
                if (noTagLines.Count == 2)
                {
                    ...
                    if (language == "ar")
                    {
                        if (arr0.EndsWith('-') && noTagLines[1].TrimStart().EndsWith('-') && arr0.Length > 1 && (LineEndChars.Contains(arr0[0]) || arr0.StartsWith("--", StringComparison.Ordinal) || arr0.StartsWith('–')))
                            return text;
                    }
                    else
                    {
                        if (arr0.StartsWith('-') && noTagLines[1].TrimStart().StartsWith('-') && arr0.Length > 1 && (LineEndChars.Contains(arr0[arr0.Length - 1]) || arr0.EndsWith("--", StringComparison.Ordinal) || arr0.EndsWith('–')))
                            return text;
                    }
