        ...
        { //[01:05.99]I've been walking in the same way as I do
            ...
            foreach (string line in lines)
            {
                if (line.StartsWith('[') && _timeCode.Match(line).Success)
                {
                    ...
                    if (parts.Length == 3)
                    {
                        var start = TimeCode.FromTimestampTokens("0", parts[0], parts[1], parts[2]);
                        var text = line.Substring(10).Trim();
                        var p = new Paragraph(start, new TimeCode(0), text);
            ...
            for (int i = 0; i < max; i++)
            {
                ...
                while (_timeCode.Match(p.Text).Success)
                {
                    ...
                    var newParagraph = new Paragraph(start, new TimeCode(0), text);
