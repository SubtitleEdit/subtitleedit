    ...
    public class SwiftTextLineNumber : SubtitleFormat
    {
        ...
        private static bool TryReadTimeCodesLine(string line, Paragraph paragraph)
        {
            ...
            if (RegexTimeCodes.IsMatch(line))
            {
                ...
                var timestamp = line.Replace("SUBTITLE:", string.Empty).Replace("TIMEIN", string.Empty).Replace("TIMEOUT", string.Empty).Replace(" ", string.Empty).Replace("\t", string.Empty);
                var tokens = timestamp.Split(':');
                paragraph.StartTime = TimeCode.FromFrameTokens(tokens[1], tokens[2], tokens[3], tokens[4]);
                paragraph.EndTime = TimeCode.FromFrameTokens(tokens[5], tokens[6], tokens[7], tokens[8]);
                return true;
            }
            return false;
        }
