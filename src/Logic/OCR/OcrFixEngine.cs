    ...
    public class OcrFixEngine
    {
        ...
        private void LoadSpellingDictionariesViaDictionaryFileName(string threeLetterIsoLanguageName, CultureInfo culture, string dictionaryFileName, bool resetSkipList)
        {
            ...
            if (threeLetterIsoLanguageName.ToLower() == "eng")
            {
                ...
                {
                    if (!namesItem.EndsWith("s", StringComparison.Ordinal))
            ...
            for (int i = 0; i < text.Length; i++)
            {
                if (" ¡¿,.!?:;()[]{}+-£\"#&%\r\n".Contains(text[i].ToString())) // removed $
                {
                    if (word.Length > 0)
                    {
                        ...
                        {
                            bool doFixWord = true;
                            if (word.Length == 1 && sb.Length > 1 && sb.ToString().EndsWith("-", StringComparison.Ordinal))
                                doFixWord = false;
                            if (doFixWord)
        ...
        private string FixCommonWordErrors(string word)
        {
            ...
            while (word.Length > 1 && word.EndsWith("\"", StringComparison.Ordinal))
            {
                post = post + "\"";
            ...
            while (word.EndsWith(",", StringComparison.Ordinal) && word.Length > 1)
            {
                post = post + ",";
                word = word.Substring(0, word.Length - 1);
            }
