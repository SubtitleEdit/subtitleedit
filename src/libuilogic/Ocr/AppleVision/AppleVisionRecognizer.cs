...
/// and Live Text use. Shared by the GUI's "Apple Vision" engine and seconv's <c>applevision</c>.
///
...
/// the remaining local options are Tesseract (brew) and PaddleOCR (pip).
///
/// Driven through objc_msgSend rather than a binding library, the same way the UI's
/// <c>MacWindowsMenuInterop</c> drives AppKit. Vision is not linked into the
...
/// </summary>
public static partial class AppleVisionRecognizer
{
    ...
    /// the answer cannot change while the process runs, and the GUI asks on every OCR window.
    /// </summary>
    ...
    /// <summary>
    /// The BCP-47 tags ("en-US", "zh-Hans") this machine's Vision can recognize, straight from
    /// the framework rather than a hard-coded list: the set grows with macOS releases, and the
    /// recognizer rejects a language it does not know. Empty when Vision is unavailable.
    /// </summary>
    public static IReadOnlyList<string> GetLanguageCodes()
    {
        lock (AvailabilityLock)
        {
            return _languageCodes ??= ReadSupportedLanguageCodes();
        }
    }

    private static List<string> ReadSupportedLanguageCodes()
    {
    ...
    /// <param name="languageCode">A tag from <see cref="GetLanguageCodes"/>. Empty means "let Vision
