...
public class BinaryOcrBitmap
{
    ...
    public BinaryOcrBitmap(NikseBitmap2 nbmp)
    {
        InitializeViaNikseBmp(nbmp);
    }

    public BinaryOcrBitmap(NikseBitmap2 nbmp, bool italic, int expandCount, string text, int x, int y)
    {
        InitializeViaNikseBmp(nbmp);
        Italic = italic;
    ...
    private void InitializeViaNikseBmp(NikseBitmap2 nbmp)
    {
        Width = nbmp.Width;
        Height = nbmp.Height;
        Colors = new byte[Width * Height];
    ...
    public SKBitmap ToOldBitmap(SKColor color)
    {
