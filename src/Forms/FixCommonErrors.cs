        ...
        private void FixDanishLetterI()
        {
            ...
            var iList = new List<Regex>
                             { // not a complete list, more phrases will come
                                 ...
                                 MyRegEx(@"(?<=\b[Aa]t) i (?:aldrig|alle bliver|alle er|alle forventer|alle gør|alle har|alle ved|alle vil|bare|bager|bruger|dræber|dræbte|fandt|fik|finder|forstår|får|hver især|ikke|ikke|kom|kommer|næsten er|næsten fik|næsten har|næsten skulle|næsten var|også får|også gør|også mener|også siger|også tror|rev|river|samarbejder|snakkede|scorer|siger|skal|skulle|ikke|siger|har|er|bager|skal|gør|får|udnyttede|udnytter|vil|ville)\b"),
                                 ...
                                 MyRegEx(@"(?<=\b[eE]r) i (?:alle|vågne|skøre|på|mætte|gået|virkelig|ved at|to|tilfredse|sultne|stadig|sikre|sikker|parate|okay|mod|med|klar|imod|her|glade|gennem|fra|der|allesammen|allerede)\b"),
                                 ...
                                 MyRegEx(@"\bForstår i\b"),
                                 MyRegEx(@"\bf[åø]r i (?:får|klø|point|ikke|fratrukket|alle|tager|kommer)\b"),
                                 ...
                                 MyRegEx(@"(?<=\b[gG]ider) i (?:ikke|godt)\b"),
                                 ...
                                 MyRegEx(@"\bhørte i det\b"),
                                 MyRegEx(@"(?<=\b[hH]ar) i (?:tabt|tænkt|spist|ordnet|nok|nogen|ikke|gjort|det|fået|ødelagt)\b"),
                                 MyRegEx(@"\bhelvede vil i?"),
                                 MyRegEx(@"\bHer har i\b"),
                                 MyRegEx(@"\bher har i\b"),
                                 MyRegEx(@"\b[Hh]older i fast\b"),
                                 MyRegEx(@"\b[Hh]older i godt fast\b"),
                                 MyRegEx(@"\bHvad fanden (?:tror|vil|har|gør) i\b"),
                                 ...
                                 MyRegEx(@"\b[Hh]vis i (?:altså|bare|forstår|får|går|ikke|lovede|lover|overholder|overtræder|slipper|taber|vandt|vinder\b"),
                                 ...
                                 MyRegEx(@"\bi (?:altid|ankomme|ankommer|bare kunne|bare havde|bare gjorde|begge er|begge gør|begge har|begge var|begge vil|behøver ikke gemme|behøver ikke prøve|behøver ikke skjule|behandlede\b"),
                                 MyRegEx(@"\bi (?:behandler|beskidte dyr|blev|blive|bliver|burde|er|fyrer af|gør|gav|gerne |giver|gjorde|hører|hørte|har|havde|igen bliver|igen burde|igen finder\b"),
                                 MyRegEx(@"\bi (?:igen gør|igen kommer|igen prøver|igen siger|igen skal|igen vil|ikke gerne|ikke kan|ikke kommer|ikke vil|kan|kom|komme|kommer\b"),
                                 MyRegEx(@"\bi (?:kunne|morer jer|må gerne|må give|må da|nåede|når|prøve|prøvede|prøver|sagde|scorede|ser|set|siger|sikkert alle|sikkert ikke gør|sikkert ikke kan|sikkert ikke vil\b"),
                                 MyRegEx(@"\bi (?:skal|skulle|små stakler|stopper|synes|troede|tror|var|vel ikke|vil\b|ville\b"),
                                 ...
                                 MyRegEx(@"\b(?<=[Ss]kal) i (?:ud|til|med|på|løfte|lave|larme|klatre|klare|ikke|have|gøre|fortælle|fordeles|fordele|dø|dele|bare|begge dø|allesammen|alle)\b"),
                                 ...
                                 MyRegEx(@"\b(?<=Så) i (?:det|at|noget)\b"),
                                 MyRegEx(@"\b[Ss]å tager i\b"),
                                 MyRegEx(@"\b(?<=[Tt]ænder) i på\b"),
                                 MyRegEx(@"\b[tT]og i (?:liften|bilen|toget)\b"),
                                 MyRegEx(@"\b(?<=[Tt]ræder i frem\b"),
                                 MyRegEx(@"\b(?<=[Tt]rr?or) i,? (?:på|jeg|det|at)\b"),
                                 MyRegEx(@"\b[Vv]ar i blevet\b"),
                                 MyRegEx(@"\b(?<=[Vv]ed) i (?:tror|mente|hvor|hvad|hvem|havde|har|ville|var|hvordan|hvorfor|hvor|hvem|hvad|ikke|er|allesammen|alle)\b"),
                                 MyRegEx(@"\b[Vv]enter i på\b"),
                                 MyRegEx(@"(?<=\b[vV]il) i (?:dræbe|besegle|være|virkeligt|smage|sikkert|se|savne|måske|møde|kaste|købe|ikke|høre|have|godt|gerne|fortryde|fjerne)\b"),
                                 MyRegEx(@"(?<=\b[vV]ille) i (?:blive|være|tro|tage|synes|sikkert|se|savne|nå|mene|komme|kaste|ikke|[hg]øre|få|dræbe)\b"),
