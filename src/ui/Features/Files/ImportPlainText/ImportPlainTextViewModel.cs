...
public partial class ImportPlainTextViewModel : ObservableObject, IClosingCleanup
{
    ...
    private async Task AlignScriptWithForcedAligner()
    {
        ...
        if (!setupVm.HandleMissingText)
        {
            await RunForcedAlignerAsync(setupVm.ExecutablePath, setupVm.AlignerModelPath);
            return;
        }

        // Transcribe first, so lines can still be located across anything the script
        // leaves out. Forced alignment on its own cannot do that - it has to place every
        // line it is given, so past an omission the following lines land on the missing
        // passage's audio.
        var whisperVm = await _windowService.ShowDialogAsync<SpeechToTextWindow, SpeechToTextViewModel>(
            Window,
            viewModel => { viewModel.Initialize(_videoFileName, -1); });
        ...
        await RunHybridAlignerAsync(setupVm.ExecutablePath, setupVm.AlignerModelPath, whisperVm.TranscribedSubtitle);
    }

    /// <summary>
    /// Locates the script against a transcription first, then forced-aligns each run of
    /// confidently matched lines for precision. The matching step tolerates text missing
    /// from either side; the alignment step is what gets timings to a few tens of ms.
    /// </summary>
    private async Task RunHybridAlignerAsync(string executable, string alignerPath, Subtitle transcription)
    {
        ...
        try
        {
            ...
            AlignProgress = "Matching script to the transcription...";
            var coarse = ScriptSyncService.SyncScript(lines, transcription);

            AlignProgress = "Refining time codes...";
            ...
            var result = await new HybridAligner(runner, audio)
                .RefineAsync(lines, coarse.DirectMatches, progress);

            RefreshSubtitlesFrom(lines);

            if (coarse.UnmatchedLines > coarse.TotalLines / 4)
            {
                await MessageBox.Show(
                    Window!,
                    Se.Language.General.Warning,
                    $"{coarse.MatchedLines} of {coarse.TotalLines} lines were found in the transcription, "
                    + $"and {result.Refined} had their time codes aligned precisely."
                    + Environment.NewLine
                    + "Lines that were not found keep approximate time codes interpolated from their "
                    + "neighbours. If the audio language or content doesn't match the script, the "
                    + "transcription may be off.",
                    MessageBoxButtons.OK,
                    MessageBoxIcon.Warning);
            }
        }
