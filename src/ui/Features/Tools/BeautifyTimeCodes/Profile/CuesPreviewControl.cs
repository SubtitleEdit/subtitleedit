...
public sealed class CuesPreviewControl : Control
{
    ...
    public override void Render(DrawingContext context)
    {
        ...
        {
            DrawLabel(context, LabelTypeface, TextBrush, 4, 4, leftEnd - 8, h - 8, 1000, GetLeftOutCueMs());
        }
        if (_rightGap <= 12 && (w - rightStart) > 40)
        {
            DrawLabel(context, LabelTypeface, TextBrush, rightStart + 4, 4, w - rightStart - 8, h - 8, GetRightInCueMs(), 5000);
        }

        // Shot change line — pale green, 3px
        context.FillRectangle(ShotChangeBrush, new Rect(center - 1.5, 0, 3, h));
    }
