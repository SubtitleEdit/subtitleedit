...
public class LlamaCppDownloadService(HttpClient httpClient) : ILlamaCppDownloadService
{
    ...
    /// </summary>
    public const string ReleaseTag = "b11149";
    ...
    /// <summary>The CUDA 13.4 build - for driver stacks built for CUDA 13.</summary>
    ...
    public async Task DownloadCudaRuntime(Stream stream, string variant, IProgress<float>? progress, CancellationToken cancellationToken)
    {
        ...
        var url = BaseUrl + (isCuda13 ? "cudart-llama-bin-win-cuda-13.4-x64.zip" : "cudart-llama-bin-win-cuda-12.4-x64.zip");
    ...
    private static string GetEngineUrl(string variant)
    {
        if (OperatingSystem.IsWindows())
        {
            return variant switch
            {
                ...
                VariantCuda13 => BaseUrl + "llama-" + ReleaseTag + "-bin-win-cuda-13.4-x64.zip",
