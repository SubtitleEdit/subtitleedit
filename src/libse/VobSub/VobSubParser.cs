...
namespace Nikse.SubtitleEdit.Core.VobSub {
    public class VobSubParser {
        ...
        private static bool IsStartOfMpeg2Pack(Stream ms, long position) {
            ...
            if (position + s_minMpeg2SectionLength >= ms.Length) return false;

            // fingerprint the mpeg2 header
            byte[] buffer = new byte[Mpeg2Header.Length];
            ms.Seek(position, SeekOrigin.Begin);
            ms.Read(buffer, 0, buffer.Length);
            if (!IsMpeg2PackHeader(buffer)) return false;
            if ((buffer[4] & 0b1100_0100) != 0b0100_0100) return false;
            // byte 6 and 8 needs xxxx_x1xx
            if ((buffer[6] & 0b0000_0100) != 0b0000_0100) return false;
            if ((buffer[8] & 0b0000_0100) != 0b0000_0100) return false;
            // byte 9 needs xxxx_xxx1
            if ((buffer[9] & 0b0000_0001) != 0b0000_0001) return false;
            // byte 12 has a hard req of xxxx_xx11
            if ((buffer[12] & 0b0000_0011) != 0b0000_0011) return false;
            // mux value can't be 0
            if (buffer[10] == 0x00 && buffer[11] == 0x00 && buffer[12] == 0x03) return false;
            // there should be at least room for the whole private header (8 bytes) + the stream id byte
            if (ms.Position + (buffer[13] & 0b111) + 9 >= ms.Length) return false;

            // fingerprint the private stream 1
            byte[] privateStreamBuffer = new byte[9];
            ms.Seek(buffer[13] & 0b111, SeekOrigin.Current);
            ms.Read(privateStreamBuffer, 0, privateStreamBuffer.Length);
            if (!IsPrivateStream1(privateStreamBuffer, 0)) return false;
            // byte 6 needs 10xx_xxxx
            if ((privateStreamBuffer[6] & 0b1100_0000) != 0b1000_0000) return false;
            // check if we have enough data for this packet
            if (ms.Position - 3 + Helper.GetEndian(privateStreamBuffer, 4, 2) > ms.Length) return false;
            // check if we have enough data for the header data + stream id
            if (ms.Position + privateStreamBuffer[8] + 1 > ms.Length) return false;
            // check for a valid private stream id.
            ms.Seek(privateStreamBuffer[8], SeekOrigin.Current);
            if (!IsSubtileStreamId(ms.ReadByte() & 0xff)) return false;
