...
namespace Nikse.SubtitleEdit.Core.SubtitleFormats
{
    public class UnknownSubtitle9 : SubtitleFormat
    {
        ...
        public override string ToText(Subtitle subtitle, string title)
        {
            ...
            {
                sb.AppendLine($"      <a class=\"caption\" starttime=\"{((long)(Math.Round(p.StartTime.TotalMilliseconds))).ToString(CultureInfo.InvariantCulture)}\" duration=\"{((long)(Math.Round(p.DurationTotalMilliseconds))).ToString(CultureInfo.InvariantCulture)}\">{HtmlUtil.RemoveHtmlTags(p.Text, true).Replace(Environment.NewLine, "<br />")}</a>");
            }
            sb.AppendLine("    </div>");
            sb.AppendLine("  </div>");
            sb.AppendLine("</div>");
